				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("Shelf_id") === oObject.Shelf_id;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].Shelf_id;
										} else {
											sCurrentId = aItemsNotInTheList[0].Shelf_id;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Shelf", {
											Shelf_id : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("Shelf_id"),
							title: oBindingContext.getProperty("Shelf_description")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Shelf (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Shelf (0)'");
							},
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("Width");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("Shelf_description");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Shelf to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Shelf (" + iExpectedLength + ")"}),
									success : function () {
