sap.ui.define([
	...
	return BaseObject.extend("shelfmaster.ui.model.GroupSortState", {
		 ...
		 * @alias shelfmaster.ui.model.GroupSortState
		 */
		...
		/**
		 * Sorts by Shelf_description, or by Width
		 *
		...
		/**
		 * Groups by Width, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Width") {
				...
				this._oViewModel.setProperty("/sortBy", "Width");

				aSorters.push(
					new Sorter("Width", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Shelf_description");
			}
